...
const { _getScripts, _findLine, _between } = require("#util");
...
const DEFAULT_USER_AGENT =
  "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html) (yt-search; https://www.npmjs.com/package/yt-search)";
...
function search(query, callback) {
  ...
  function callback_with_retry(err, data) {
    if (err) {
      if (
        _options._attempts > (_options.MAX_RETRY_ATTEMPTS || MAX_RETRY_ATTEMPTS)
      ) {
        ...
        const wait_ms =
          Math.pow(2, n - 1) * (_options.RETRY_INTERVAL || RETRY_INTERVAL);
...
function _parseSearchResultInitialData(responseText, callback) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const isLive =
      ...
      _jp.query(item, "$..badges..label").join("").toUpperCase().trim() ===
        "LIVE NOW" ||
      _jp
        ...
        case "video":
          {
            ...
            const viewsCount = Number(
              viewCountText.split(/\s+/)[0].split(/[,.]/).join("").trim(),
            );
            ...
            const description =
              _jp
                .query(item, "$..detailedMetadataSnippets..snippetText..text")
                .join("") ||
        ...
        case "list":
          {
            const thumbnail =
              _normalizeThumbnail(
                _jp.value(item, "$..primaryThumbnail..url"),
              ) ||
            ...
            const video_count_label = _jp.value(
              item,
              "$..overlays..thumbnailBadges..text",
            );
            ...
            result = {
              ...
              videoCount:
                Number(_parseNumbers(video_count_label)[0]) || video_count,
        ...
        case "channel":
          {
            ...
            let about_channel =
              _jp
                .query(item, "$..channelRenderer..descriptionSnippet..text")
                .join("") || "";
            ...
            const channel_verified =
              ...
              channel_verified_label
                .toLowerCase()
                .trim()
                .search(/[\s_]?verified/) >= 0;
        ...
        case "live":
          {
            ...
            const watchingLabel =
              ...
              _jp.query(item, "$..viewCountText..simpleText").join("") ||
              "0";

            const watchCount = Number(
              watchingLabel.split(/\s+/)[0].split(/[,.]/).join("").trim(),
            );
            ...
            const scheduledEpochTime = _jp.value(
              item,
...
function _parseVideoInitialData(responseText, callback) {
  ...
  const description =
    _jp
      .query(idata, "$..detailedMetadataSnippets..snippetText..text")
      .join("") ||
...
function _parsePlaylistInitialData(responseText, callback) {
  ...
  let viewCount = 0;
  try {
    const viewCountLabel = _jp.value(
      json,
