        ...
        case "channel":
          {
            ...
            // const url = _jp.value( item, '$..navigationEndpoint..url' )
            const url =
              _jp.value(item, "$..navigationEndpoint..url") || "/user/" + title;
...
function _parseVideoInitialData(responseText, callback) {
  ...
  const sentimentBar =
    ...
    _jp
      .value(idata, "$..sentimentBar..tooltip")
      .split(/[,.]/)
      .join("")
      .split(/\D+/);
...
function _parsePlaylistInitialData(responseText, callback) {
  ...
  const thumbnailUrl = _jp.value(idata, "$..thumbnail..url");
  const thumbnail = _normalizeThumbnail(thumbnailUrl);

  const description = _jp.value(idata, "$..microformat..description") || "";
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      ...
      const thumbnail = _normalizeThumbnail(
